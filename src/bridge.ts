...
contextBridge.exposeInMainWorld(
	'ipcRenderer', {
		...
		receive: (channel: string, func: (...args: any[]) => void) => {
			const validChannels = ['show', 'blur', 'set-image', 'stop-loading', 'show-notification', 'claude-response-init', 'claude-response-chunk', 'claude-response-done', 'claude-response-error'];
			if (validChannels.includes(channel)) {
